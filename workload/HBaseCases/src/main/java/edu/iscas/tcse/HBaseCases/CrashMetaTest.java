...
public class CrashMetaTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			stopMeta();
			Logger.log(testName, " stop meta server.");
			//create table
			...
			startMeta();
			Logger.log(testName, " start meta server.");
			...
			Result row3 = HBaseUtils.getRow(MyTableSchema.myTable, "3");
			if(row3 == null) {
				String info = Logger.log(testName, " the talbe "+MyTableSchema.myTable+" does not contain row3");
				reportFailure(info);
			} else {
				Logger.log(testName, " the table "+MyTableSchema.myTable+" 's row3: "+row3.toString());
			}
			//delete table
