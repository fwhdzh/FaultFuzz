...
public class BuildBaseData {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//create table
			boolean createtable = MyTableSchema.createBigTable(rowcount);//row start from 0, contains two column families
			boolean blcrun = HBaseUtils.balance(true);
			Logger.log(testName, "run balance ..."+blcrun);
			if(blcrun) {
				try {
