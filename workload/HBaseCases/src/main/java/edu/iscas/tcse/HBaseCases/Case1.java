		...
		try {
			...
			if(check) {
				...
				if(tables == null || tables.size()== 0) {
					reportFailure("Test case1 Table not created error: table 'test':"+tables.size());
				} else {
					boolean bug = true;
			...
			if(check) {
				...
				if(rst == null || !rst.equals("value1")) {
					reportFailure("Test case1 Put 'row1-cf1-a' failed:"+rst.toString());
				}
			}
			...
			if(check) {
				...
				if(rst == null || !rst.equals("value2")) {
					reportFailure("Test case1 Put 'row1-cf1-b' failed:"+rst.toString());
				}
			}
			...
			if(check) {
				...
				if(rst == null || !rst.equals("value3")) {
					reportFailure("Test case1 Put 'row1-cf2-a' failed:"+rst.toString());
				}
			}
			...
			if(check) {
				...
				if(rst == null || !rst.equals("value4")) {
					reportFailure("Test case1 Put 'row2-cf3-b' failed:"+rst.toString());
				}
			}
			
			String cell = HBaseUtils.getCell(tName, row1, cf1, "row1-cf1-b");
			System.out.println("Test case1 get cell at table '"+tName+"', row '"+row1+"', column "
					+"'"+cf1+"', key 'row1-cf1-b': "+cell);
			ResultScanner scanner = HBaseUtils.getScanner(tName);
			System.out.println("Test case1 Test case1 going to scan table '"+tName+"'");
			Iterator<Result> iter = scanner.iterator();
			while(iter.hasNext()) {
				Result s = iter.next();
				if(s != null && s.current() != null) {
					System.out.println("**********");
					System.out.println("Test case1 timestamp:"+s.current().getTimestamp());
					System.out.println("Test case1 RowArray:"+new String(s.current().getRowArray()));
					System.out.println("Test case1 FamilyArray:"+new String(s.current().getFamilyArray()));
					System.out.println("Test case1 ValueArray:"+new String(s.current().getValueArray()));
					System.out.println("Test case1 QualifierArray:"+new String(s.current().getQualifierArray()));
				}
			}
			HBaseUtils.deleteRow(tName, row1);
			cell = HBaseUtils.getCell(tName, row1, cf1, "row1-cf1-b");
			System.out.println("Test case1 Test case1 get cell at table '"+tName+"', row '"+row1+"', column "
					+"'"+cf1+"', key 'row1-cf1-b': "+cell);
			HBaseUtils.deleteTable(tName);
			System.out.println("Test case1 Test case1 deleted table '"+tName+"'");
			...
			if(check) {
				...
				if(tables != null) {
					for(TableDescriptor t:tables) {
						if(t.getTableName().getNameAsString().equals(tName)) {
							reportFailure("Test case1 The table 'test' was not deleted:"+tables.size());
							break;
						}
					}
				}
