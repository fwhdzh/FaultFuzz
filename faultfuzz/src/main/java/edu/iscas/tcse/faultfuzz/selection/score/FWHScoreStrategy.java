package edu.iscas.tcse.faultfuzz.selection.score;
...
public class FWHScoreStrategy {
    ...
    private static Map<String, Set<FaultPoint>> computeNodeToFaultsMap() {
        ...
        for (FaultPoint fp: SelectionInfo.testedFault) {
            String nodeIp = null;
            if (fp.stat.equals(FaultStat.CRASH) || fp.stat.equals(FaultStat.REBOOT)) {
                nodeIp = fp.tarNodeIp;
            }
            if (fp.stat.equals(FaultStat.NETWORK_DISCONNECT) || fp.stat.equals(FaultStat.NETWORK_CONNECT)) {
    ...
    public static List<EntryAndScore> computeTotalScore(List<QueueEntry> queue) {
        ...
        for (int i = 0; i < queue.size(); i++) {
            ...
            // long similarMutationBehaviorScore = emp.scoreMap.get("similarMutationBehavior");
            // es.score = (basicSocre + perfSocre) * recoverySocre / nodeSymmetrySocre / similarMutationBehavior;
